...
public class GameProfileRepository {
	...
	public void findProfilesByNames(String[] names, ProfileLookupCallback callback) {
		...
		while(failCount < MAX_FAIL_COUNT && !criteria.isEmpty()) {
			try {
				ProfileSearchResultsResponse response = (ProfileSearchResultsResponse) URLUtils.makeRequest(URLUtils.constantURL(SEARCH_PAGE_URL + page), request, ProfileSearchResultsResponse.class);
				failCount = 0;
				error = null;
